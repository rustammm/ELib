...
function detectBrowser() {
	...
	// IE version check (IE10+ supports file input data-transform=\"input-control\" onChange and drag & drop uploads)
...
function processFileUploads(e,isDrop,isFolder,globalFiles) {
	...
	if (xmlhttp.upload) {
		...
		if (globalBrowser != "chrome" || (isFolder == 0 && isDrop == 0)) {
			...
			// Set the files input data-transform=\"input-control\" field if not drag & drop
			if (globalFiles == '')
...
function displayUploadButtons() {
	...
	if (globalBrowser == "ie9-") {
		html += '<input data-transform=\"input-control\" type="file" name="uploadFile" id="uploadFile" multiple onChange="processFileUploads(event,0,0,\'\')"> ';
		html += '<input data-transform=\"input-control\" type="button" value="'+lang_btn_upload_file+'" onClick="uploadFileCheck(event)" class="'+adjustButtonWidth(lang_btn_upload_file)+'">';
	}
	
	// Non-IE9 Upload File
	if (globalBrowser != "ie9-") {
		html += '<input data-transform=\"input-control\" type="button" value="'+lang_btn_upload_files+'" onClick="fileChoose(event)" class="'+adjustButtonWidth(lang_btn_upload_files)+'">';
	}
	...
	html += '<div id="repeatUploadDiv" style="visibility: hidden; display: none">';
		html += '<div class="floatLeft10">';
			html += '<input data-transform=\"input-control\" type="button" value="'+lang_btn_upload_repeat+'" onclick="processFileUploads(event,0,0,\'\')" class="'+adjustButtonWidth(lang_btn_upload_repeat)+'">';
		html += '</div>';
	html += '</div>';
	...
	if (globalBrowser == "chrome") {
		html += '<div class="floatLeft">';
			html += '<input data-transform=\"input-control\" type="button" value="'+lang_btn_upload_folder+'" onClick="dirChoose(event)" class="'+adjustButtonWidth(lang_btn_upload_folder)+'">';
			html += '<div class="uploadHiddenDiv">';
				html += '<input data-transform=\"input-control\" type="file" name="uploadDir" id="uploadDir" onChange="processFileUploads(event,0,1,\'\')" webkitdirectory directory>';
			html += '</div>';
		html += '</div>';
	}
	...
	if (globalBrowser != "ie9-") {
		html += '<div class="uploadHiddenDiv">';
			html += '<input data-transform=\"input-control\" type="file" name="uploadFile" id="uploadFile" onChange="processFileUploads(event,0,0,\'\')"';
